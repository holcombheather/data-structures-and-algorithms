...
const screenForNames = (arr) => {
  const regex = /^(Mr\.|Mrs\.|Ms\.|Dr\.) [a-zA-Z]+(\s[a-zA-Z]+)*$/;
  return arr.filter(str => !str.startsWith(' ') && regex.test(str)).map(name => name.trim().replace(/\s{2,}/g, ' '));
};
...
let biggerThanLuke = (arr) => {
  const lukeMass = arr.find(char => char.name === 'Luke Skywalker')?.mass;
  if (!lukeMass) {
    return '';
  }
  const heavierChars = arr.filter(char => parseFloat(char.mass) > parseFloat(lukeMass));
  return heavierChars.map(char => char.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  for (let i = 0; i < 3; i++) {
    if (
      helpCheck(board, i, 0, i, 1, i, 2) ||
      helpCheck(board, 0, i, 1, i, 2, i)
    ) {
      return true;
    }
  }
