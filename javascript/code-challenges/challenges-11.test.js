...
const count = (target, input) => {
  return input.reduce((total, subArray) => {
    return total + subArray.reduce((subTotal, num) => {
      return num === target ? subTotal + 1 : subTotal;
    }, 0);
  }, 0);
};
...
let findMaleAndFemale = (data) => {
  ...
  let characterNames = filteredCharacters.map(character => character.name);
